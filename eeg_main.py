...
#     wandb.init(
#         project=f"mwl-{args.dataset}-{args.task}",
#         config={
...
#     wandb.watch(model, log="all", log_freq=0)
...
#     test_model.cuda()
#     with torch.no_grad():
#         dummy_input = torch.randn(1, 126, 31, 30).cuda()
...
# #             print(f"LOOCV: Testing on subject {test_subj}, training on the rest.")
# #             train_x, train_y = [], []
# #             test_x, test_y = subject_data[test_subj]['x'], subject_data[test_subj]['y']

# #             for train_subj in subjects:
# #                 if train_subj != test_subj:
# #                     train_x.append(subject_data[train_subj]['x'])
# #                     train_y.append(subject_data[train_subj]['y'])
...
# #             print(f"Train label data shape: {train_y.shape}, Test label data shape: {test_y.shape}")
            
# #             # Normalize features
# #             normalize_seizure_features([train_x, test_x])

# #             # Generate dataloaders
# #             datasets = generate_dataloader_seizure([train_x, test_x], [train_y, test_y], args)

# #             # Train and evaluate
# #             tr_metrics, te_metrics = train_eeg(args, datasets, mwl_levels, index=subjects.index(test_subj))
# #             test_results.append(te_metrics[0]['test_acc'])

# #         # Report LOOCV results
# #         print("LOOCV Results:")
# #         print(f"Test Accuracies: {test_results}")
# #         print(f"Mean Accuracy: {np.mean(test_results):.4f}, Std Dev: {np.std(test_results):.4f}")
...
#             if int(test_subj) in [1015, 1031, 1032, 1039, 1064]: 
#                 # 1005, 1011, 1028, 1042, 1048, 1058, 1066: eyesopen v nback
#                 # 1033: 0v2v4
#                 # 1084: 1v3v5
#                 # 1060: baeline
#                 continue
...
#             # Exclude test subject and randomly pick 5 validation subjects
...
#             # Train and evaluate
#             start_time = time.time()
#             tr_metrics, te_metrics = train_eeg(args, datasets, mwl_levels, subject=test_subj, index=subjects.index(test_subj))
...
#     elif args.dataset == 'EYESOPEN_V_NBACK':
...
#     elif args.dataset == 'EASY_V_MEDIUM':
...
#     elif args.dataset == 'MEDIUM_V_HARD':
...
#     elif args.dataset == 'EASY_V_MEDIUM_V_HARD':
